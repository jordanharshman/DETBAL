...
revrxnmessage <- reactive({
  ...
  for(i in 1:nrow(indprodall)){
    ...
    # Next if and loop is to detect if there should be reversible reactions that have been written as such:
    #  Reaction 1: A + A = B
    #  Reaction 2: B = 2A
    # These should be detected as reversible
    if(sum(duplicated(indspecies[i,])) >= sum(indspecies[i,] == "BLANK")){ # if there's more than expected duplicated "BLANKS"
      notblank <- indspecies[i,][indspecies[i,] != "BLANK"]                # species that aren't BLANK
      coef <- data.frame(table(notblank), stringsAsFactors = FALSE)        # get number of duplicated times
      newspecies <- paste0(coef$Freq, coef$notblank)                       # build a new species putting coefficient in front
      
      # Quick loop to get rid of the extra "1" in front of species
      for(j in 1:length(newspecies)){
        if(substr(newspecies[j], 1, 1) == "1"){
          newspecies[j] <- substr(newspecies[j], 2, nchar(newspecies[j]))
        }
      }
      
      notblank <- matrix(newspecies, ncol = length(newspecies))            # write new species as matrix
      indspecies[i,] <- cbind(notblank, matrix(rep("BLANK", 10 - ncol(notblank)), nrow = 1)) # combine with BLANKS
    }
  }
...
revrxnrem <- reactive({
  ...
  for(i in 1:nrow(indprodall)){
    ...
    if(sum(duplicated(indspecies[i,])) >= sum(indspecies[i,] == "BLANK")){
      notblank <- indspecies[i,][indspecies[i,] != "BLANK"]
      coef <- data.frame(table(notblank), stringsAsFactors = FALSE)
      newspecies <- paste0(coef$Freq, coef$notblank)
      for(j in 1:length(newspecies)){
        if(substr(newspecies[j], 1, 1) == "1"){
          newspecies[j] <- substr(newspecies[j], 2, nchar(newspecies[j]))
        }
      }
      notblank <- matrix(newspecies, ncol = length(newspecies))
      indspecies[i,] <- cbind(notblank, matrix(rep("BLANK", 10 - ncol(notblank)), nrow = 1))
    }
  }
